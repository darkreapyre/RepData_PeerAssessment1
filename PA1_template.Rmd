...
These missing values potentially introduce bias into the analysis performed in the previous sections. By looking at the summary, the missing values are in the Steps column of the data frame. TO investigate wether the data is skewed by these missing values, a new data frame is created, substituting any missing values with the already calculated mean for the particula interval.
...
df3 <- merge(tmpMean, df, by = "interval")
sort <- order(df3$date, as.numeric(df3$interval))
df3 <- df3[sort, ]

##vInterate through the rows with a for loop, replacing the NA value with the
## mean vlaue
for (i in 1:nrow(df3)) {
        if (is.na(df3$steps[i])) {
                df3$steps[i] <- df3$mean[i]
        } else {
                df3$steps[i] <- df3$steps[i]
        }
}
...
newsumByDate <- tapply(df3$steps, df3$date, sum)
...
## subset the data by weekday and weekend
subWeekend <- subset(df2, df2$weekday == "weekend")
subWeekday <- subset(df2, df2$weekday == "weekday")

## Get the the average no. of steps per interval for each subset
meanByWeekday <- tapply(subWeekday$steps, subWeekday$interval, mean)
meanByWeekend <- tapply(subWeekend$steps, subWeekend$interval, mean)

## Create a unique data frame for plotting
library(lattice)
x <- data.frame(interval = unique(subWeekday$interval),
                mean = as.numeric(meanByWeekday),
                day = rep("weekday", length(meanByWeekday)))
y <- data.frame(interval = unique(subWeekend$interval),
                mean = as.numeric(meanByWeekend),
